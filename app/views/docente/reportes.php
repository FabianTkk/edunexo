...
<div style="padding-top:1rem">
    ...
    <form method="POST" class="data-card">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:1rem">
            ...
            <div class="form-group"><label class="form-label">Semana</label><input class="form-input no-icon" type="date" name="periodo_semana" value="<?= htmlspecialchars($editando ? $reporteEdicion['periodo_semana'] : date('Y-m-d')) ?>" required></div>
            ...
            <div class="form-group"><label class="form-label">Ausencias</label><input class="form-input no-icon" type="number" min="0" name="dias_ausente" value="<?= (int)($editando ? $reporteEdicion['dias_ausente'] : 0) ?>"></div>
    ...
    <div class="data-card"><div class="data-card-header"><span class="data-card-title">Historial de mis reportes</span></div><div style="overflow-x:auto"><table class="tbl"><thead><tr><th>Estudiante</th><th>Semana</th><th>Calificación</th><th>Ausencias</th><th>Creado</th><th>Acciones</th></tr></thead><tbody>
        <?php if (!$reportes): ?><tr class="empty-row"><td colspan="6">Aún no creaste reportes.</td></tr><?php else: foreach($reportes as $r): $puedeEditar=strtotime($r['created_at']) >= time() - (48*3600); ?><tr>
            <td style="color:var(--text-primary);font-weight:600"><?= htmlspecialchars($r['nombre_completo']) ?></td><td><?= date('d/m/Y',strtotime($r['periodo_semana'])) ?></td><td><?= htmlspecialchars($r['calificacion_general']) ?></td><td><?= (int)$r['dias_ausente'] ?></td><td><?= date('d/m/Y H:i',strtotime($r['created_at'])) ?></td>
