...
class DocenteGestionController {
    ...
    public function reportes(): void {
        ...
        $stmt=$db->prepare("SELECT r.*,e.nombre_completo,s.id AS solicitud_pendiente,(SELECT COUNT(DISTINCT a.fecha) FROM asistencias a WHERE a.estudiante_id=r.estudiante_id AND a.presente=0 AND a.justificada=0 AND a.fecha BETWEEN DATE_SUB(r.periodo_semana, INTERVAL WEEKDAY(r.periodo_semana) DAY) AND DATE_ADD(DATE_SUB(r.periodo_semana, INTERVAL WEEKDAY(r.periodo_semana) DAY), INTERVAL 4 DAY)) AS ausencias_reales FROM reportes r JOIN estudiantes e ON e.id=r.estudiante_id LEFT JOIN solicitudes_cambio_reportes s ON s.reporte_id=r.id AND s.docente_id=r.usuario_id AND s.estado='pendiente' WHERE r.usuario_id=? ORDER BY r.periodo_semana DESC,r.created_at DESC"); $stmt->execute([$uid]); $reportes=$stmt->fetchAll();
        $reporteEdicion = null;
